...
Requires Python 3.11 or newer. Install the editable package and the integrations you plan to use:

```sh
python3 -m pip install -e '.[gmail,secure,tui]'
```
...
The default configuration is written to `~/.maily/config.toml`. It includes the local Ollama endpoint, the `gemma4:e2b` model, a 20-second timeout, and the required categories. Edit configuration values there; maily preserves existing configuration on later launches.
