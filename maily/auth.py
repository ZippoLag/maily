...
def authenticate(client_file: Path, database: Database, credentials: CredentialStore):
    ...
    client, email, token_json = build_authenticated_client(client_file, credentials.get(token_key))
    ...
    with database.transaction() as connection:
        ...
        if existing and existing[0] != email:
            raise RuntimeError("v1 supports exactly one Gmail account; remove the existing account before authenticating another")
        connection.execute(
