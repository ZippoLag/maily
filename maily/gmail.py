...
def parse_date_range(spec: str, now: datetime | None = None) -> tuple[datetime, datetime]:
    ...
    match = re.fullmatch(r"last (\d+) (day|days|week|weeks|month|months|year|years)", spec)
    if match:
        ...
        if unit == "month":
            ...
            month = (month - 1) % 12 + 1
            return current.replace(year=year, month=month, day=1, hour=0, minute=0, second=0, microsecond=0), current
        if unit == "year":
            return current.replace(year=current.year - count, month=1, day=1, hour=0, minute=0, second=0, microsecond=0), current
    if spec == "this month":
    ...
    if match:
        ...
        if unit == "year":
            return _GMAIL_EPOCH, current.replace(year=current.year - count, month=12, day=31, hour=23, minute=59, second=59, microsecond=999999)
    raise ValueError(f"Unsupported date range: {spec}")


_GMAIL_EPOCH = datetime(2004, 1, 1, tzinfo=timezone.utc)
        ...
        for spam in (False, True):
            ...
            while request is not None:
                ...
                for item in response.get("messages", []):
                    raw_request = self.service.users().messages().get(userId="me", id=item["id"], format="full")
