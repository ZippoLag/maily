...
def split_date_range(start: datetime, end: datetime, chunk_size: str = "day") -> list[tuple[datetime, datetime]]:
    """Split [start, end] into date-based chunks (day/week/month/year)."""
    if chunk_size not in CHUNK_SIZES:
        raise ValueError(f"Invalid chunk size: {chunk_size} (expected one of {', '.join(CHUNK_SIZES)})")
    chunks: list[tuple[datetime, datetime]] = []
    cursor = start
    while cursor <= end:
        if chunk_size == "day":
            chunk_end = cursor.replace(hour=23, minute=59, second=59, microsecond=999999)
        elif chunk_size == "week":
            chunk_end = (cursor + timedelta(days=6)).replace(hour=23, minute=59, second=59, microsecond=999999)
        elif chunk_size == "month":
            chunk_end = (cursor.replace(day=28) + timedelta(days=4)).replace(day=1) - timedelta(microseconds=1)
        else:  # year
            chunk_end = cursor.replace(month=12, day=31, hour=23, minute=59, second=59, microsecond=999999)
...
def scan(
    ...
    try:
        ...
        for chunk_index, (chunk_start, chunk_end) in enumerate(chunks):
            ...
            messages = gmail_client.fetch_messages(chunk_start, chunk_end, include_read=include_read)
        ...
        return ScanResult(started, iso_now(), status, all_messages, results, errors, category_names=list(classifier.categories))
