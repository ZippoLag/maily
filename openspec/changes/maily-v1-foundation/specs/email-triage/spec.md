...
The system SHALL allow category rules to be configured and SHALL evaluate deterministic rules before invoking an inference provider.
...
- **WHEN** deterministic rules cannot classify a message
- **THEN** the system passes the message to the configured inference provider if available
